...
setupPoolEditor() {
  ...
  _applyLivePreviewFromDirty() {
    ...
    // Lightweight preview strategy:
    // - length/width: scale X/Y (keeps meshes/materials/sims intact)
    // - depths: scale Z by max-depth ratio (approximate while dragging)
    // - other params: no live preview; rely on final rebuild
    let sx = 1, sy = 1, sz = 1;

    if (this._live.dirty.has("length") || this._live.dirty.has("width")) {
      ...
      // Keep previous scaling if only Z changes? We keep X/Y at 1 for stability.
      sx = this.poolGroup.scale.x || 1;
      sy = this.poolGroup.scale.y || 1;
    }

    // Depth changes are expensive to preview correctly (would require vertex updates),
    // and scaling Z breaks semantics (it moves shallow/deep/steps/coping together).
    // So we DO NOT live-preview depth sliders; we rely on the accurate rebuild on release.
    sz = 1;


    // Apply scaling preview (top datum at z=0 stays stable because group pivot is at z=0)
    this.poolGroup.scale.set(sx, sy, sz);

    // While dragging we avoid re-running void cutting / grass rebuild every frame.
    // But we can cheaply update the ground void less often by piggybacking on this tick.
    ...
    // Clear only the params we handled for preview; keep others dirty for rebuild
